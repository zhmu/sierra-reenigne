...
impl Picture {
        ...
        loop {
            ...
            match PicOp::try_from(opcode) {
                ...
                Ok(PicOp::RelativePatterns) => {
                    ...
                    pic.draw_pattern(&coord, col, priority, control, pattern_code, pattern_nr);
                ...
                Ok(PicOp::RelativeMediumLines) => {
                    ...
                    while !is_next_byte_a_command(&mut rdr) {
                        ...
                        pic.offset_rect(&mut rect);
                        pic.clamp_rect(&mut rect);
                        pic.draw_line(&rect, col, priority, control);
                        prev_coord = coord;
                    }
                },
                ...
                Ok(PicOp::Fill) => {
                    while !is_next_byte_a_command(&mut rdr) {
                        let coord = get_abs_coords(&mut rdr)?;
                        pic.dither_fill(&coord, col, priority, control);
                   }
                },
                Ok(PicOp::SetPattern) => {
                    let code = rdr.read_u8()?;
                ...
                Ok(PicOp::RelativeMediumPatterns) => {
                    ...
                    pic.draw_pattern(&prev_coord, col, priority, control, pattern_code, pattern_nr);
                    while !is_next_byte_a_command(&mut rdr) {
                        update_pattern_nr(&mut rdr, pattern_code, &mut pattern_nr);
                        let coord = get_rel_coords_med(&mut rdr, &prev_coord)?;
                        pic.draw_pattern(&coord, col, priority, control, pattern_code, pattern_nr);
                ...
                Ok(PicOp::X) => {
                    ...
                    match PicOpX::try_from(opcodex) {
                        //Ok(PicOpX::SetPaletteEntry) => { return Err(anyhow!("PicOpX")); },
                        Ok(PicOpX::SetPalette) => {
                            let palette_index = rdr.read_u8()? as usize;
                            if palette_index >= EGA_PALETTE_COUNT {
                                return Err(anyhow!("invalid palette index {}", palette_index));
                            }
                            for n in 0..EGA_PALETTE_SIZE {
                                palette[palette_index][n] = rdr.read_u8()?;
                            }
                        },
                        // Ok(PicOpX::Mono0) => { return Err(anyhow!("todo PicOpX Mono0")); },
                        // Ok(PicOpX::Mono1) => { return Err(anyhow!("todo PicOpX Mono1")); },
                        // Ok(PicOpX::Mono2) => { return Err(anyhow!("todo PicOpX Mono2")); },
                        // Ok(PicOpX::Mono3) => { return Err(anyhow!("todo PicOpX Mono3")); },
                        // Ok(PicOpX::EmbeddedView) => { return Err(anyhow!("todo PicOpX EmbeddedView")); },
                        // Ok(PicOpX::SetPriorityTable) => { return Err(anyhow!("todo PicOpX SetPriorityTable")); },
                        Ok(v) => { return Err(anyhow!("todo PicOpX unimplemented {:?}", v)); },
                        Err(_) => {
                            return Err(anyhow!("unrecognized opcode x {}", opcodex));
                        }
                    }
                },
                Ok(PicOp::End) => {
                ...
                Err(_) => { return Err(anyhow!("unrecognized opcode {}", opcode)); }
            }
        }
    }
    ...
    pub fn new_pic1(data: &[u8], load_palette: bool, palette: &mut [ u8; 768 ]) -> Result<Picture> {
        ...
        if pic_hdr.cel_count > 0 {
            let pic_hdr2 = VGAPicHeader2::unpack_from_slice(&data[32..42]).unwrap();
            let cel_offset = pic_hdr2.visual_header_offset as usize;
            ...
            for y in 0..(cel.height as usize) {
                for x in 0..(cel.width as usize) {
                    ...
                    pic.visual[(pen_y + y) * SCREEN_WIDTH as usize + pen_x as usize + x] = pixel;
                }
            }
        }

        let mut rdr = Cursor::new(&data[pic_hdr.vector_offset as usize..]);
        ...
        loop {
            ...
            match VGAPicOp::try_from(opcode) {
                ...
                Ok(VGAPicOp::ShortBrush) => {
                    return Err(anyhow!("unrecognized opcode ShortBrush"))
                },
                Ok(VGAPicOp::MediumBrush) => {
                    return Err(anyhow!("obsolete opcode MediumBrush"))
                },
                Ok(VGAPicOp::AbsoluteBrush) => {
                    return Err(anyhow!("obsolte opcode AbsoluteBrush"))
                },
                Ok(VGAPicOp::ShortLines) => {
                    ...
                    while !is_next_byte_a_command(&mut rdr) {
                        ...
                        pic.offset_rect(&mut rect);
                        pic.clamp_rect(&mut rect);
                        pic.draw_line(&rect, c_color, p_color, v_color);
                        prev_coord = coord;
                    }
                },
                ...
                Ok(VGAPicOp::Fill) => {
                    while !is_next_byte_a_command(&mut rdr) {
                        let coord = get_abs_coords(&mut rdr)?;
                        pic.dither_fill(&coord, c_color, p_color, v_color);
                    }
                },
                Ok(VGAPicOp::SetBrushSize) => {
                    return Err(anyhow!("obsolete opcode SetBrushSize"))
                },
                Ok(VGAPicOp::Special) => {
                    let opcodex = rdr.read_u8()?;
                    match VGAPicOpSpecial::try_from(opcodex) {
                        Ok(v) => { return Err(anyhow!("unimplemented special {:?}", v)) },
                        Err(_) => {
                            return Err(anyhow!("unrecognized opcode x {}", opcodex))
                        }
                    }
